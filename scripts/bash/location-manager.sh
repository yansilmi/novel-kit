...
LOCATIONS_DIR="$REPO_ROOT/world/locations"
STATE_FILE="$REPO_ROOT/.novelkit/memory/config.json"
TEMPLATE_FILE="$REPO_ROOT/.novelkit/templates/location.md"
...
get_next_location_id() {
    local highest=0
    if [ -d "$LOCATIONS_DIR" ]; then
        for file in "$LOCATIONS_DIR"/location-*.md; do
            [ -f "$file" ] || continue
...
find_location() {
    ...
    if [ -f "$LOCATIONS_DIR/$search.md" ]; then
        echo "$search"
        return
    fi
    
    if [ -f "$LOCATIONS_DIR/location-$search.md" ]; then
    ...
    for file in "$LOCATIONS_DIR"/location-*.md; do
        ...
        if [ -f "$file" ]; then
            ...
            name=$(grep -E "^# 地点档案：" "$file" | sed 's/# 地点档案：//' | head -1 | xargs)
            if [ -z "$name" ]; then
                 name=$(grep -E "^# Location Profile:" "$file" | sed 's/# Location Profile: //' | head -1 | xargs)
            fi
...
action_new() {
    ...
    file="$LOCATIONS_DIR/$id.md"
    
    if [ -f "$TEMPLATE_FILE" ]; then
        cp "$TEMPLATE_FILE" "$file"
        # Replace placeholders
        current_date=$(date +"%Y-%m-%d")
        sed -i "s/\[LOCATION_NAME\]/$name/g" "$file"
        sed -i "s/\[NAME\]/$name/g" "$file"
        sed -i "s/\[DATE\]/$current_date/g" "$file"
        sed -i "s/\[LOCATION_ID\]/$id/g" "$file"
...
action_show() {
    local search="$*"
    local id=$(find_location "$search")
    
    if [ -z "$id" ]; then
        echo "{"action":"show","success":false,"error":"Location not found: $search"}" >&2
        exit 1
    fi
...
action_update() {
    ...
    id=$(find_location "$first_word")
    
    if [ -z "$id" ]; then
        echo "{"action":"update","success":false,"error":"Location not found: $first_word"}" >&2
        exit 1
    fi
...
action_delete() {
    ...
    if [ -z "$id" ]; then
        echo "{"action":"delete","success":false,"error":"Location not found: $search"}" >&2
        exit 1
    fi
